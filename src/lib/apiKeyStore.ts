...
export async function createApiKey(name: string, createdBy?: string): Promise<{ key: ApiKey; plaintext: string }> {
  ...
  await dbRun(
    `INSERT INTO api_keys (id, name, key_prefix, key_hash, created_by) VALUES (?, ?, ?, ?, ?)`,
    [id, (name || '').trim() || 'Unbenannt', key_prefix, key_hash, createdBy || ''],
  );
  const key = (await dbGet<ApiKey>(
    `SELECT id, name, key_prefix, created_at, created_by, last_used_at, revoked FROM api_keys WHERE id = ?`,
    [id],
  ))!;
...
export async function listApiKeys(): Promise<ApiKey[]> {
  return dbAll<ApiKey>(
    `SELECT id, name, key_prefix, created_at, created_by, last_used_at, revoked FROM api_keys ORDER BY created_at DESC`,
  );
}
...
export async function verifyApiKey(plaintext: string | null | undefined): Promise<ApiKey | null> {
  ...
  const row = await dbGet<ApiKey>(
    `SELECT id, name, key_prefix, created_at, created_by, last_used_at, revoked FROM api_keys WHERE key_hash = ?`,
    [key_hash],
  );
