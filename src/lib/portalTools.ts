/**
 * portalTools.ts — Werkzeug-Registry des Portals (gemeinsam genutzt).
 * ─────────────────────────────────────────────────────────────────────────────
 * Die Werkzeuge (Events, Anfragen, Kunden, Angebote, Aufgaben) wurden aus dem
 * MCP-Server (TASK-00125) herausgelöst, damit sie zwei Aufrufer teilen:
 *   – den MCP-Server (src/lib/mcpServer.ts) für externe KI-Clients per API-Key
 *   – die persönliche Faltin-KI im KI-Arbeitsplatz (/working-dashboard)
 * Die Aufrufer unterscheiden sich nur im ToolContext (Name für created_by, PDF-Links).
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
export interface ToolContext {
  /** Anzeigename des Aufrufers für created_by-Felder, z.B. "MCP: Claude Desktop" oder "Faltin-KI (Nils)". */
  actor: string;
  /** Öffentliche Basis-URL des Portals. */
  base: string;
  /** Link auf das Angebots-PDF (MCP: mit Key in der URL, Arbeitsplatz: Admin-Route mit Session). */
...
export async function callTool(name: string, args: Record<string, unknown>, ctx: ToolContext): Promise<unknown> {
  switch (name) {
    ...
    case 'create_offer': {
      ...
      const calc = await createCalculation(input, snapshot, ctx.actor);
    ...
    case 'create_invoice_from_offer': {
      ...
      return {
        ...
        pdf_url: ctx.invoicePdfUrl(inv.id),
