...
export async function applyPgSchemaEnhancements(): Promise<void> {
  {
    ...
    // Urlaubsplanung 2.0: Genehmigungsweg, Stellvertretung, Entscheid-Kommentar
    try { await pool.query(`ALTER TABLE employees ADD COLUMN IF NOT EXISTS approver_id text`); } catch { /* ignore */ }
    try { await pool.query(`ALTER TABLE vacation_requests ADD COLUMN IF NOT EXISTS substitute_id text`); } catch { /* ignore */ }
    try { await pool.query(`ALTER TABLE vacation_requests ADD COLUMN IF NOT EXISTS decision_comment text NOT NULL DEFAULT ''`); } catch { /* ignore */ }
    ...
    try {
      ...
      await pool.query(`ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS scopes text NOT NULL DEFAULT 'all'`);
