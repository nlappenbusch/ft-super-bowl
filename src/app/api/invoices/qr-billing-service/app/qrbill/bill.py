...
class QRBill:
    ...
    def draw_bill(self, dwg, horiz_scissors=True):
        ...
        if self.additional_information:
            y_pos += 1
            grp.add(dwg.text(self.label("Additional information"), (margin, mm(y_pos)), **receipt_head_font))
            y_pos += line_space
            ...
            for info in wrap_infos(additional_information):
                ...
                grp.add(dwg.text(info, (margin, mm(y_pos)), **self.font_info))
                y_pos += line_space
        ...
        if self.amount:
            ...
            self.draw_blank_rect(
                dwg, grp, x=add_mm(margin, mm(25)), y=add_mm(currency_top, mm(-2)),
        ...
        grp.add(dwg.text(self.label("Amount"), (add_mm(payment_left, mm(14)), currency_top), **payment_head_font))
        grp.add(dwg.text(self.currency, (payment_left, add_mm(currency_top, mm(5))), **self.font_info))
        if self.amount:
            grp.add(dwg.text(
                format_amount(self.amount),
                (add_mm(payment_left, mm(14)), add_mm(currency_top, mm(5))),
                **self.font_info
            ))
        else:
            self.draw_blank_rect(
                dwg, grp, x=add_mm(RECEIPT_WIDTH, margin, mm(14)), y=add_mm(currency_top, mm(3)),
