...
import { suggestForTitles, parseCf7Line, type MatchItem } from '@/lib/cf7Match';
 ...
 *   { forms: [{id?, title, pages?}] }      – strukturiert (WP-Plugin); pages = Titel der Seiten mit dem Formular
...
export async function POST(request: Request) {
  try {
    ...
    if (Array.isArray(body.lines)) {
      ...
      items = body.forms.map((f: { id?: unknown; title?: unknown; pages?: unknown }) => ({
        id: f.id != null ? String(f.id) : undefined,
        title: String(f.title || ''),
        pages: Array.isArray(f.pages) ? f.pages.slice(0, 25).map((p) => String(p || '')) : undefined,
      }));
