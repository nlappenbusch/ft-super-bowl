...
interface LageplanMapProps {
  pins: Pin[];
  pinIcons?: PinIcon[];
  /** Free-text location (e.g. "SoFi Stadium, Inglewood, US") — auto-geocoded when no valid pins are set. */
  geocodeQuery?: string | null;
  /** Label shown on the auto-generated venue marker. */
...
function getMarkerHtml(pin: Pin, iconImage: string | null | undefined): string {
  ...
  return `
    <div style="position:relative;display:flex;flex-direction:column;align-items:center;font-family:system-ui,-apple-system,sans-serif;transform:translate(-50%,-100%);">
      ...
      ${label ? `<div style="margin-top:3px;background-color:#0f2742;color:#ffffff;border:1.5px solid rgba(255,255,255,0.25);border-radius:6px;padding:4px 9px;font-size:11px;font-weight:700;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,0.35);">${label}</div>` : ''}
    </div>
  `;
}

export default function LageplanMap({ pins, pinIcons = [], geocodeQuery = null, venueLabel = null }: LageplanMapProps) {
  const mapRef = useRef<HTMLDivElement>(null);
  const mapInstanceRef = useRef<any>(null);
  const initCalledRef = useRef(false);
  const [status, setStatus] = useState<'loading' | 'ready' | 'empty' | 'nokey'>('loading');
  ...
  useEffect(() => {
    ...
    if (validPins.length === 0 && !geocodeQuery) {
      setStatus('empty');
      return;
    }
    ...
    function initMap() {
      ...
      if (validPins.length > 0) {
        ...
        // ── Auto-geocode from venue/location ──
        const geocoder = new gm.Geocoder();
        geocoder.geocode({ address: geocodeQuery }, (results: any, geoStatus: string) => {
          if (geoStatus === 'OK' && results && results[0]) {
            ...
            new CustomHTMLOverlay(loc, getMarkerHtml(venuePin, null), map);
            setStatus('ready');
          } else {
            setStatus('empty');
          }
    ...
    <div className="w-full h-full rounded-2xl overflow-hidden border border-white/10 shadow-lg relative min-h-[450px] bg-[#143047]">
      ...
      {status === 'empty' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-white text-center">
          ...
          <p className="text-sm text-gray-300">Keine Standortdaten für die Karte vorhanden.</p>
        </div>
      )}
    </div>
  );
